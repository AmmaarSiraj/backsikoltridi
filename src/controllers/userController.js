...
const ajukanAkun = async (req, res) => {
  ...
  const { username, password, nama_lengkap, jabatan, nip_nik } = req.body;
  const level = "user";
  ...
  try {
    ...
    const insertSql =
      "INSERT INTO pengajuan_akun (username, password, level, nama_lengkap, jabatan, nip_nik) VALUES (?, ?, ?, ?, ?, ?)";
    const [result] = await db.query(insertSql, [username, password, level, nama_lengkap, jabatan, nip_nik]); // ✅ PAKAI AWAIT
